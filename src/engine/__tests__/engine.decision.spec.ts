...
describe("engine decision", () => {
	...
	it("chooses first match and emits multi-match", async () => {
		...
		const events: EngineEvent[] = onEvent.mock.calls.map(
			(c) => c[0] as EngineEvent
		);
		...
		expect(
			completed.some(
				(e) => e.type === "nodeCompleted" && e.nodeId === "dec"
			)
		).toBe(true);
	});
	...
	it("falls back to default when no match", async () => {
		...
		const reg2 = mergeRegistry(
			coreRegistry,
			createRegistry({ comparators: [neq] })
		);
		...
		const completed = events.filter((e) => e.type === "nodeCompleted");
		const nodeIds = completed.map((e) =>
			e.type === "nodeCompleted" ? e.nodeId : ""
		);
